<template>
  <q-page class="q-px-md q-pb-md q-pt-none">
    <ListHeader :title="$t( 'menu.soundFragments' )" :quantity="store.getPagination.itemCount" :show-back="false"
    ...
    <q-table :rows="items" :columns="columns" row-key="id" flat :selected="selectedRows"
      ...
      <template v-slot:top>
        <q-input v-model="search" dense clearable debounce="300" :placeholder="$t( 'common.search' )"
          @update:model-value="onSearch" class="col-6" />
        <q-space />
...
function handleDelete() {
  ...
  $q.dialog( {
    title: 'Delete',
    message: `Are you sure you want to delete ${selectedRows.value.length} item(s)?`,
    cancel: true,
    persistent: true
  } ).onOk( () => {
    loading.value = true
